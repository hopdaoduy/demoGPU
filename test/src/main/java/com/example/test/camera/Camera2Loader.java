...
public class Camera2Loader extends CameraLoader {
    public interface Listener{
        void sendByte(Bitmap bitmap) throws InterruptedException;
    }
    ...
    public Camera2Loader(Activity activity, Listener listener) {
        mActivity = activity;
                ...
                if (reader != null) {
                    ...
                    if (image != null) {
                        if (mOnPreviewFrameListener != null) {
                            ...
                            Log.d(TAG,"ImageByte"+data.toString());
                            Size size = chooseOptimalSize();
                            YuvImage img = new YuvImage(data, ImageFormat.NV21, size.getWidth(), size.getHeight(), null);
                            ByteArrayOutputStream stream = new ByteArrayOutputStream();
                            img.compressToJpeg(new Rect(0, 0, size.getWidth(), size.getHeight()), 80, stream);
                            Bitmap bitmap = BitmapFactory.decodeByteArray(stream.toByteArray(), 0, stream.size());
                            try {
                                mListener.sendByte(bitmap);
